#!/usr/bin/env bash
# Benchmark PR #28243 (rebuilt d1a92352c on master) on Qwen3.8-Flash-Next
# Fresh 6-prompt unique corpus at n_max=1 and n_max=2
set -euo pipefail
...
HEAD="/home/kchauhan/models/unsloth/Qwen3.8-Flash-Next-GGUF/MTP/mtp-Qwen3.8-Flash-Next-shared-Q8_0.gguf"
PROMPTS="$REPO/bench-models/prompts/qwen38-flash-next-unique.jsonl"
PORT=8024
LOGDIR="$REPO/bench-models/logs/results"
STAMP="$(date +%Y%m%d-%H%M%S)"
OUTDIR="$LOGDIR/pr28243-unique-$STAMP"
mkdir -p "$OUTDIR"
...
run_arm() {
  ...
  echo "================================================================="
  echo "== Arm: $arm (n_max=$n_max, head: $(basename "$HEAD"))"
  echo "================================================================="
  ...
  GGML_CUDA_NO_PINNED=1 GGML_CUDA_GRAPH_OPT=1 nohup taskset -c 0-11 "$BIN" \
    "${COMMON_FLAGS[@]}" "${spec_args[@]}" > "$log" 2>&1 &
  local srv_pid=$!
...
run_arm "28243-nmax1" 1
run_arm "28243-nmax2" 2

echo
echo "================================================================="
echo "PR #28243 Results saved in $OUTDIR"
echo "================================================================="
